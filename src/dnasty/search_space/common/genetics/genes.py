...
from dnasty.search_space.common.components import Flatten
from dnasty.utils.config import Config
from dnasty.utils.types import size_2_opt_t, size_2_t

_allowed_activations = nn.modules.activation.__all__


def validate_feature(name: str, value: Any, allowed_range: Container | Iterable):
    """
    ...
    def validate(v, r):
        if v not in r:
            ...
            warnings.warn(
                f"{name} ({v}) not allowed, adjusting to nearest value: {closest_val}",
                stacklevel=3,
            )
            return closest_val
        return v
...
def create_conv_block_sequence(cfg: Config) -> list:
    ...
    ] + [MaxPool2dGene.from_random()]


class GeneBase(abc.ABC):  # noqa: B024 - concrete hooks, ABC marks intent
    """
    ...
    _feature_ranges: dict[str, Any] = {}
    __module_sig_cache: dict[str, inspect.Signature] = {}
    ...
    def from_random(cls):
        ...
        for param_name, param in init_params.items():
            ...
            if param_name in cls._feature_ranges:
                ...
                search_space = package_path[package_path.index("search_space") + 1]
                try:
                    genetics_module = importlib.import_module(
                        name=".genetics", package=f"dnasty.search_space.{search_space}"
                    )
                    if hasattr(genetics_module, param.annotation.__name__):
                        gene_class = getattr(genetics_module, param.annotation.__name__)
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if hasattr(cls, name):
            ...
            raise AttributeError(f"No attribute {name} in {type(self).__name__}")
...
class LinearBlockGene(GeneBase):
    ...
    Args:
        ...
        activation (str | None, optional): The activation function to be
         applied. Must be one of the allowed activations, or None for no
         activation (the output layer). Defaults to "ReLU".
    ...
    def from_random(cls):
        ...
        gene.out_features = random.randrange(
            min(LinearBlockGene._feature_ranges["out_features"]), gene.in_features + 1
        )
    ...
    def __init__(
        ...
        if not isinstance(dropout, bool):
            raise TypeError(f"dropout must be a boolean, not {type(dropout).__name__}.")

        out_features = validate_feature(
...
class ConvBlock2dGene(GeneBase):
    ...
    def __init__(
        ...
        in_channels = validate_feature(
            "in_channels", in_channels, self._feature_ranges["in_channels"]
        )
        out_channels = validate_feature(
            "out_channels", out_channels, self._feature_ranges["out_channels"]
        )
        kernel_size = validate_feature(
            "kernel_size", kernel_size, self._feature_ranges["kernel_size"]
        )
    ...
    def num_params(self):
        ...
        if isinstance(self.kernel_size, int):
            return self.out_channels * (1 + self.in_channels * self.kernel_size**2)
...
class MaxPool2dGene(GeneBase):
    ...
    _feature_ranges = {"kernel_size": set(range(2, 5)), "stride": set(range(2, 5))}

    def __init__(self, kernel_size: size_2_t, stride: size_2_opt_t = None) -> None:
        kernel_size = validate_feature(
            "kernel_size", kernel_size, self._feature_ranges["kernel_size"]
        )
