...
import sys
sys.path.append('P:/')
from Arrhythmia_generator_1D import DataGenerator
...
with open('1DCNN_3_Beat_Train_Labels_OHE.pkl', 'rb') as f:
...
print(Y_labels)
# Using Kfold to split the data up
kfold = StratifiedKFold(n_splits=5, shuffle=True)
fold_num = 1
fold_val_accuracies = []
for train, val in kfold.split(X_final, Y_labels):
    ...
    # Model architecture
    input_img = Input(shape = (2800,2), name = 'CWTimage')
    ...
    CAmaxpool1 = GlobalMaxPooling1D()(maxpool2)
    CAavpool1 = GlobalAveragePooling1D()(maxpool2)
    # Define two dense layers to function as a MLP with one hidden layer
    CA1_dense1 = Dense(2, activation='relu')
    ...
    Chat_out1 = Multiply()([maxpool2, CA1_output])
    
    # CONV BLOCK 2
    conv3 = Conv1D(64, kernel_size = 5, activation='relu')(Chat_out1)
    ...
    # Use both global max and average pooling
    CAmaxpool2 = GlobalMaxPooling1D()(maxpool6)
    CAavpool2 = GlobalAveragePooling1D()(maxpool6)
    # FDefine two dense layers to function as a MLP with one hidden layer
    CA2_dense1 = Dense(8, activation='relu')
    CA2_dense2 = Dense(128, activation='relu')
    # Feed both pooling into MLP
    ...
    # Now perform element wise multiplication with input feature map
    Chat_out2 = Multiply()([maxpool6, CA2_output])
    ...
    callback_list = [lrate]
    train_generator = DataGenerator(X_train, Y_train_OHE, **params)
    val_generator = DataGenerator(X_val, Y_val_OHE, **params_val)
    print(model.summary())
    print('=============================================')
    print('TRAINING FOR FOLD {}: '.format(fold_num))    
    history = model.fit_generator(generator=train_generator,
                              epochs=50, validation_data=val_generator,
                              verbose=1)
    """, callbacks = callback_list"""
    scores_generator = DataGenerator(X_val, Y_val_OHE, **params_test)
    ...
    print('=============================================')
    print('ACCURACY: ')
    acc = accuracy_score(Y_val_OHE, flat_predictions)
    print(acc)
    fold_val_accuracies.append(acc)
    # Save model
    model.save('fold_model_saves/fold_{}_model_save.h5'.format(fold_num))
    with open('fold_history_saves/fold_{}_history.pkl'.format(fold_num), 'wb') as f:
...
fold = 1
for accuracy in fold_val_accuracies:    
    file.write('Fold {} val. accuracy: {}\n'.format(fold, accuracy))
    fold += 1
file.close()
