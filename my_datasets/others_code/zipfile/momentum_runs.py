...
mom_values = [0.5,0.6,0.7,0.8,0.9,0.99]
model_accuracies = []
for mom in mom_values:
    ...
    sgd = optimizers.SGD(learning_rate=0.001, momentum=mom, decay=1e-6, nesterov = True)
    ...
    print('=============================================')
    print('TRAINING FOR Momentum = {}: '.format(mom))
    ...
    # Save model
    model.save('model_saves/mom_{}_model_save.h5'.format(mom))
    with open('history_saves/mom_{}_history.pkl'.format(mom), 'wb') as f:
        pickle.dump(history.history,f)
    print('*** MOMENTUM =  ', mom, ' MODEL SAVED ***')

# save accuracies to a file
file = open('accuracy.txt', 'w')
for i in range(len(mom_values)):    
    file.write('Momentum = {} val. accuracy: {}\n'.format(mom_values[i], model_accuracies[i]))
file.close()
