...
class Trainer:
    def __init__(self,
        ...
        self.data = self._load_data(data_path, reference_path, ksplit)
        self.ksplit = ksplit
    ...
    def run(self):
        for fold, triplet in enumerate(self.data):
            ...
            eval_loader = DataLoader(dataset=triplet[1], batch_size=self.config.eval.batch_size, shuffle=True)
            test_loader = DataLoader(dataset=triplet[2], batch_size=len(triplet[2]), shuffle=False)
