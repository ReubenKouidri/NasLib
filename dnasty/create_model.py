...
def create_model(genome):
    ...
    for i, rb_type in enumerate(rb_types):  # e.g. (0, 1) : (2, 2)
        if rb_type > 1:  # select the RBs with > 1 conv layer
            for j in range(1, rb_type):  # j = 1
                genome.chromosomes[i].chromosomes[f'in_planes_{j}'] = genome.chromosomes[i].chromosomes[
                    f'out_planes_{j - 1}']
    ...
    if num_rbs > 1:
        for i, j in enumerate(range(1, num_rbs)):  # e.g. range(1, 4) = (1, 2, 3)
            genome.chromosomes[j].chromosomes["in_planes_0"] = genome.chromosomes[i].chromosomes[
                f"out_planes_{rb_types[i] - 1}"]
    ...
    for i, res in enumerate(genome.chromosomes[:num_rbs]):
        if res.chromosome_type == 'RB1':
            rb1 = ResBlock1(in_planes_0=res.chromosomes['in_planes_0'], out_planes_0=res.chromosomes['out_planes_0'],
                            se_ratio=res.chromosomes['r_ratio'], conv_kernel_size_0=res.chromosomes['conv_ker_0'],
                            att_kernel_size=res.chromosomes['att_ker'], mp_ker=res.chromosomes['mp_ker'], cbam=True,
                            spatial=False)
            ...
            raise TypeError("should either be 'RB1', 'RB2'!")
