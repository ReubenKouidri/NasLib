...
class Genome:
    ...
    def from_random(cls, cfg: Config) -> Genome:
        return cls.from_sequence(create_gene_sequence(cfg))
    ...
    def append(self, gene: GeneBase) -> None:
    ...
    def sync_genes(self) -> None:
        ...
        if not isinstance(first_gene, ConvBlock2dGene) or \
    ...
    def linear_params(self) -> int:
        return sum(gene.num_params for gene in self.genes.values() if
                   isinstance(gene, LinearBlockGene))
