...
class TestLinearGene(unittest.TestCase):
    def setUp(self):
        self.gene = LinearBlockGene(
            in_features=10, out_features=20, activation="ReLU", dropout=True
        )

    def _test_within_ranges(self, gene):
        self.assertIn(gene.in_features, LinearBlockGene._feature_ranges["in_features"])
        self.assertIn(
            gene.out_features, LinearBlockGene._feature_ranges["out_features"]
        )
