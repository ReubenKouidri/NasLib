...
from dnasty.search_space.cbam import ChannelAttention, ChannelAttentionGene
...
class TestChannelAttentionGene(unittest.TestCase):
    def setUp(self):
        self.se_ratio = 4
        self.in_channels = 2
        self.gene = ChannelAttentionGene(
            se_ratio=self.se_ratio, in_channels=self.in_channels
        )
    ...
    def test_mutate(self):
        ...
        self.assertIn(
            self.gene.se_ratio, ChannelAttentionGene._feature_ranges["se_ratio"]
        )
    ...
    def test_express(self):
        ...
        self.assertEqual(
            module.mlp[1].out_features, max((self.gene.in_channels // 4), 1)
        )
        self.assertEqual(
            module.mlp[3].in_features, max((self.gene.in_channels // 4), 1)
        )
